...
// Get Article Info
$stmt = $conn->prepare("SELECT * FROM `article` INNER JOIN `author` ON `article`.id_author = `author`.user_id  WHERE `article_id` = ?");
$stmt = $conn->prepare("SELECT * FROM `article`,`users`,`author` WHERE `article`.id_author = `users`.userid AND `author`.user_id = `users`.userid AND `article_id` = ?");
                    ...
                    <div class="card bg-light my-3">
                        ...
                        <ul class="list-group list-group-flush">
                            <?php foreach ($articles as $article) : ?>
                                <li class="list-group-item"><a href="single_post.php?id=<?= $article['article_id'] ?>"><?= $article['article_title'] ?></a></li>
