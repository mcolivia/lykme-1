    ...
    if (isset($_POST["update"])) {
        ...
        switch ($type) {
            ...
            case "author":
                ...
                echo "
                       <script>alert('User Records updated Successfully!')</script>
                ...
                case "profile":
                    // Update DataBase
                    $username = test_input($_POST["username"]);
                    $password = test_input($_POST["opassword"]);
                    $hashed_password = test_input($_POST["fopassword"]);
                    ...
                    if(!password_verify($password, $hashed_password)){
                        echo "
                       <script>alert('Old Password does not Matched')</script>                      
                   ";
                  // header("Location: ../profile.php", true, 301);
                   exit();
                    ...
                    if($npassword != $cnpassword){
                        echo "
                       <script>alert('New Pasword and Confirm Password Does not Match')</script>                      
                   ";
                  // header("Location: ../profile.php", true, 301);
                   exit();
                    }
                    ...
                    try {
                        $sql = "UPDATE `author`
                            ...
                            $stmt->execute([$fullName, $description, $email, $twitter, $github, $linkedin, $imageName, $urlId]);
                        if(!empty($npassword)){
                            $password_enc = password_hash($npassword, PASSWORD_DEFAULT);
                            $sql2 = "UPDATE `users` SET `password`= ? WHERE `userid` = ?";
                            $stmt2 = $conn->prepare($sql2); 
                            $stmt2->execute([$password_enc, $urlId]);
                            //var_dump($stmt2);
                           // exit();
                           }                
